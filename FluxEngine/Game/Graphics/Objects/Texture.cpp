...
Texture::Texture(ID3D11Device* device, const std::string& filePath, aiTextureType type)
{
	...
	if (StringTools::GetFileExtension(filePath) == ".dds")
		hr = DirectX::CreateDDSTextureFromFile(device, StringTools::StandardToWide(filePath).c_str(), this->texture.GetAddressOf(), this->textureView.GetAddressOf());
	else
		hr = DirectX::CreateWICTextureFromFile(device, StringTools::StandardToWide(filePath).c_str(), this->texture.GetAddressOf(), this->textureView.GetAddressOf());
...
Texture::Texture(ID3D11Device* device, const uint8_t* pData, size_t size, aiTextureType type)
{
	...
	EXCEPT_IF_FAILED(hr, "Failed to create Texture from memory");
}
...
void Texture::InitializeColorTexture(ID3D11Device* device, const Color* colorData, UINT width, UINT height, aiTextureType type)
{
	...
	EXCEPT_IF_FAILED(hr, "Failed to initialize texture from color data");
	...
	EXCEPT_IF_FAILED(hr, "Failed to create shader resource view from texture generated");
}
